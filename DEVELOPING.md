...
| 界面里那排图标的位置 | `assets/ae2/screens/me_salvager.json` 的 `rarityFilter` 条目：格子 18×18、间距 0（= 18 的列距，与槽位网格同拍），5 格共 90 px，起点 (80,16)，正好压在第 5~9 列槽位上方、右边缘与 9 格槽位行（8..170）齐平；过滤列表是**四行 36 格**（`FILTER_SLOTS = 36`），行首在 y=40/58/76/94 |
...
- 界面贴图是烘焙式的：槽位底纹（`#8B8B8B` 底、上/左 `#373737`、下/右 `#FFFFFF` 的凹槽）直接画在 `me_salvager.png` 上，
  AE2 不会另外画槽底。面板尺寸 **176×250**（AE2 自己的接口是 176×205，多出来的高度给了四行过滤槽）。注意 250 单位在 GUI scale 2 下需要**窗口高度 ≥ 500px**，850×480 这种小窗口会被 AE2 顶到屏幕外（客户端自检因此会把测试窗口调大）。
